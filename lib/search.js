...
function Yt_Search(search) {
  return new Promise(async (resolve, reject) => {
    try {
      let yts = require("yt-search")
      let hasil = await yts(search)
      resolve(hasil.all)
    } catch (err) {
      console.error(err)
    }
...
function Lirik(search) {
  return new Promise(async (resolve, reject) => {
    try {
      const google = require('google-it')
      let rus = await google({ 'query': `lyrics ${search} genius` })
      let ros = []
      for (let g of rus) {
      ...
      if (ros.every((x) => x === undefined)) return resolve({ developer: '@xorizn', mess: `Cannot find ${search} song` })

      const { data } = await axios.get(ros[0], {
        headers: {
      ...
      const $ = cheerio.load(data)
      const lk = $('div[data-lyrics-container="true"]').html()
      if (lk === undefined || lk === null) {
        resolve({ developer: '@xorizn', mess: `Cannot find ${search} song` })
      } else {
        resolve(lk.replace(/<br>/g, '\n').replace(/<[^>]*>/g, ''))
      }
    } catch (err) {
      console.error(err)
    }
...
function GoogleS(search) {
  return new Promise(async (resolve, reject) => {
    try {
      let google = require('google-it')
      let hasil = await google({ 'query': search })
      resolve(hasil)
    } catch (err) {
      console.error(err)
    }
...
function GoogleImages(search) {
  return new Promise(async (resolve, reject) => {
    try {
      let gis = require('g-i-s')
      gis(search, async (error, result) => {
        if (error) {
...
function KodePos(query) {
  return new Promise(async (resolve, reject) => {
    try {
      const { data } = await axios.get(`https://nomorkodepos.com/?s=${query}`, { timeout: 5000 });
      const $ = cheerio.load(data);
      const tableRows = $('table.pure-table.pure-table-horizontal > tbody > tr');
      const _data = [];

      tableRows.each((i, row) => {
        const cells = $(row).find('td');
        const province = cells.eq(0).text().trim();
        const city = cells.eq(1).text().trim();
        const subdistrict = cells.eq(2).text().trim();
        const village = cells.eq(3).text().trim();
        const postalcode = cells.eq(4).text().trim();

        _data.push({ province, city, subdistrict, village, postalcode });
      });
