...
function Upakara() {
  return new Promise(async (resolve, reject) => {
    try {
      let { data } = await axios.get('https://kalenderbali.org/')
      const $ = cheerio.load(data)
      let datas = []
      $("#left-sidebar > div:nth-child(2) > ul > li").each((u,i) => {
        let prnama = $(i).text().trim()
        datas.push(prnama)
      })
      resolve(datas)
    } catch (err) {
...
function KodePos(query) {
  return new Promise(async (resolve, reject) => {
    try {
      const { data } = await axios.get(`https://kodeposku.com/api/search?q=${query}`);
      resolve(data.data);
    } catch (err) {
      console.error(err);
      reject(err);
...
function KodePos2(query) {
  return new Promise(async (resolve, reject) => {
    try {
      const { data } = await axios.get(`https://carikodepos.com/?s=${query}`);
      const $ = cheerio.load(data)
      let _data = []
      $('div.table.table-striped > table > tbody > tr').each((u,i) => {
        let _doto = []
        $(i).find('td').each((l,o) => {
          _doto.push($(o).text().trim())
        })
        _data.push({
          province: _doto[0],
          regency: _doto[1],
